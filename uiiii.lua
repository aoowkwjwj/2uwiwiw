...
function SectionInfo:AddDropdown(DropdownName:string, DropdownInfo:{any}, Default:any, DropdownMax:number, callback, multi:boolean)
    multi = multi or false
    callback = callback or function() end

    local valnow = multi and (typeof(Default) == "table" and Default or {}) or Default
    ...
    Dropdown.Size = UDim2.new(0.98, 0, 0.5, 0)
    Dropdown.BackgroundTransparency = 1
    Dropdown.ZIndex = 5
    Dropdown.Parent = SectionFrame

    local Text = Instance.new("TextLabel", Dropdown)
    Text.Name = "Text"
    Text.Text = DropdownName or "Dropdown"
    Text.TextColor3 = Color3.fromRGB(255, 255, 255)
    Text.TextScaled = true
    Text.BackgroundTransparency = 1
    Text.Size = UDim2.new(0.96, 0, 0.5, 0)
    Text.Font = Enum.Font.GothamBold
    Text.ZIndex = 6

    local DropFrame = Instance.new("Frame", Dropdown)
    DropFrame.Name = "DropFrame"
    DropFrame.Size = UDim2.new(0.96, 0, 0.55, 0)
    ...
    Header.Name = "Header"
    Header.Size = UDim2.new(0.925, 0, 1, 0)
    ...
    Icon.Position = UDim2.new(0.94, 0, 0.5, 0)
    Icon.AnchorPoint = Vector2.new(0.5, 0.5)
    Icon.BackgroundTransparency = 1
    Icon.Image = "rbxassetid://7733717447"
    Icon.ZIndex = 7

    local SelectText = Instance.new("TextLabel", Header)
    SelectText.Name = "SelectText"
    SelectText.Text = join(valnow)
    SelectText.TextColor3 = Color3.fromRGB(255, 255, 255)
    SelectText.TextTransparency = 0.5
    SelectText.TextScaled = true
    SelectText.Size = UDim2.new(0.86, 0, 0.55, 0)
    SelectText.Position = UDim2.new(0.437, 0, 0.5, 0)
    ...
    local ScrollingFrame = Instance.new("ScrollingFrame", DropFrame)
    ScrollingFrame.Size = UDim2.new(1, 0, 5.65, 0)
    ...
    local function toggleui(val)
        if val then
            ...
            TweenService:Create(Icon,TweenInfo.new(0.3),{ImageColor3 = Alc.Config.MainColor}):Play()
        else
    ...
    Header.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            toggleui(DropFrame.Size.Y.Scale <= 0.55)
        end
    end)

    local function getbutton(txt)
        local Btn = Instance.new("TextButton")
        Btn.Size = UDim2.new(0.95, 0, 0.5, 0)
        ...
        local Selected = Instance.new("ImageLabel", Btn)
        Selected.Size = UDim2.new(0.1, 0, 0.6, 0)
        Selected.Position = UDim2.new(0.9, 0, 0.2, 0)
        Selected.BackgroundTransparency = 1
        Selected.Image = "rbxassetid://3944680095"
        Selected.ImageTransparency = 1
        Selected.ZIndex = 7

        local function toggle(val)
            TweenService:Create(Selected, TweenInfo.new(0.3), {
                ImageTransparency = val and 0 or 1,
                ImageColor3 = val and Alc.Config.MainColor or Color3.fromRGB(0,0,0)
            }):Play()
        end
    ...
    for _, v in ipairs(DropdownInfo) do
        local Button, ToggleEffect = getbutton(v)
        Button.Parent = ScrollingFrame

        local selected = false
        if multi and table.find(valnow, v) then
            selected = true
            ToggleEffect(true)
        elseif not multi and valnow == v then
            selected = true
            ToggleEffect(true)
        end
        ...
        Button.MouseButton1Click:Connect(function()
            if multi then
                if selected then
                    selected = false
                    ToggleEffect(false)
                    table.remove(valnow, table.find(valnow, v))
                else
                    if #valnow < DropdownMax then
                        selected = true
                        ToggleEffect(true)
                        table.insert(valnow, v)
                    end
                end
                SelectText.Text = join(valnow)
                callback(valnow, v)
            else
                for _, obj in ipairs(connectlist) do
                    if obj.Toggle then obj.Toggle(false) end
                end
                selected = true
                valnow = v
                ToggleEffect(true)
        ...
        table.insert(connectlist, {Toggle = ToggleEffect})
    end
end				
...
local Window = Alc:NewWindow('Blue X Hub','Steal a Brainrot','rbxassetid://6886692638')
...
Section:AddDropdown(
    "Select Skills",                -- Tên hiển thị
    {"Z", "X", "C", "V"},           -- Danh sách giá trị
    {"Z"},                          -- Danh sách mặc định
    3,                              -- DropdownMax (ví dụ tối đa 3)
    function(selectedList, item)    -- callback trả về list và item vừa bấm
        print("Selected list:", table.concat(selectedList, ", "))

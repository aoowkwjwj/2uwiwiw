...
local G = Window:AddMenu('Garden',"Vườn",'ticket','tab')
local E = Window:AddMenu('Event',"Sự Kiện",'ticket','tab')
local P = Window:AddMenu('Pet',"Thú Cưng",'ticket','tab')
local S = Window:AddMenu('Shop',"Cửa Hàng",'ticket','tab')

local F = G:AddTab('Farm','','list')

local Farm = F:AddSection('Plant')

local seed = {}
for _, item in ipairs(game.Players.LocalPlayer.Backpack:GetChildren()) do
    if item:IsA("Tool") and item.Name:lower():find("seed") then
        table.insert(seed, item.Name)
    end
end

Farm:AddDropdown('Select Seed Plant',seed,nil,1,function(value)
	print(list,item)
end)

Farm:AddToggle('Auto Plant Seed',false,function(value)
	print(v)
end)

Farm:AddToggle('Auto Plant All Seed',false,function(value)
	print(v)
end)

local S = G:AddTab('Sell','','list')

local Sell = S:AddSection('Sell')

Sell:AddSlider('Time Sell',{Min = 0.2, Max = 10, Default = 1, VALUE = 's'},function(value)
	print(v)
end)

Sell:AddToggle('Auto Sell All',false,function(value)
	print(v)
end)

Sell:AddButton('Sell All',function()
	print('TP!')
end)

local D = E:AddTab('Dino Event','','list')

local Dino = D:AddSection('Dino Event')

local ReplicatedStorage = game:GetService("ReplicatedStorage")
local petFolder = ReplicatedStorage:WaitForChild("Assets"):WaitForChild("Animations"):WaitForChild("PetAnimations")
...
Dino:AddDropdown('Select Pet Craft Dino Egg',petNames,nil,1,function(value)
	print(list,item)
end)

Dino:AddToggle('Auto Craft Dino Egg',false,function(value)
	print(v)
end)

Dino:AddToggle('Auto Claim Dino Egg',false,function(value)
	print(v)
end)

local dinoevent = {"Mutation Spray Amber","Ancient Seed Pack","Dino Crate","Archaeologist Crate"}

Dino:AddDropdown('Select Craft Dino Event',dinoevent,nil,1,function(value)
	print(list,item)
end)

Dino:AddToggle('Auto Craft Dino Event',false,function(value)
	print(v)
end)

local Sh = S:AddTab('Shop','','list')

local Shop = Sh:AddSection('Shop')

local seedbuy = {"Carrot","Strawberry","Blueberry","Loquat","Tomato","Pineapple","Bell Pepper","Green Apple","Feijoa","Avocado","Cauliflower","Pitcher Plant","Sugar Apple","Banana","Prickly Pear","Watermelon","Kiwi","Rafflesia"}

Shop:AddDropdown('Select Seed',seedbuy,nil,1,function(value)
	print(list,item)
end)

Shop:AddDropdown('Select Egg',dinoevent,nil,1,function(value)
	print(value)
end)

local gear = {"Watering Can","Trowel","Recall Wrench","Basic Sprinkler","Advanced Sprinkler","Godly Sprinkler","Lightning Rod","Tanning Mirror", "Master Sprinkler", "Magnifying Glass","Cleaning Spray","Favorite Tool","Harvest Tool","Friendship Pot"}

Shop:AddDropdown('Select Gear',gear,nil,1,function(value)
	_G.ChonGear = value
end)

Shop:AddToggle('Auto Buy Gear',false,function(value)
	_G.BuyGear = value
end)
